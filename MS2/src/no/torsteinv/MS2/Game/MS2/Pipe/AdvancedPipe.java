...
public class AdvancedPipe extends Pipe implements Selectable {
	// up, down, left, right
	...
	public AdvancedPipe(int X, int Y, int Player, FactoryBuilding Home) {
		super(X, Y, Player);
	...
	public AdvancedPipe(HashMap<String, Object> Properties,
	...
	public static void updateOthersDirections() {
		for (Entity p : Main.Entities)
			if (p instanceof AdvancedPipe)
				((AdvancedPipe) p).generateDirections();
	}
	...
	private static Direction genRandomDir(Direction from,
			Direction... direction) {
		/*
		 * if (direction.length >= 3) return from; Random r = new Random(); int
		 * ri = r.nextInt(4); for (Direction i : direction) if
		 * (i.getDirectrionInteger() == ri) return genRandomDir(from,
		 * direction); return Direction.valueOf(ri);
		 */
