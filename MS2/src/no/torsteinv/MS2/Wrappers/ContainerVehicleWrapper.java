...
public abstract class ContainerVehicleWrapper extends Vehicle implements
		Container {

	public ContainerVehicleWrapper(float x, float y, int player,
			String TextureTag, int discoverPower) {
		super(x, y, player, TextureTag, discoverPower);
	}

	public ContainerVehicleWrapper(HashMap<String, Object> Properties,
	...
	public boolean contains(BaseItem i, int q) {
		for (ItemStack is : this.content)
			if (is.getQuantity() > q && is.getType().equals(i))
				return true;
		return false;
	}
