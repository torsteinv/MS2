...
public class TemplateDecoder {
			...
			for (int y = 0; y < 10; y++)
				if (d.getRGB(x, y) != 0xFFFFFF) {
					...
					if (mt.toString().equals("Pipe"))
						...
						fe = new InputMachine(x, y, Parent, MainItemList.none,
								1);
	...
	public static void buy(Template t) throws Exception {
		...
		while (scan.hasNext()) {
			...
			Class<? extends FactoryElement> c = (Class<? extends FactoryElement>) EntityList.MasterEntityTypeList
					.get(s[0]);
			if (c == null)
				continue;
	...
	private static FactoryElement at(int x, int y,
			CopyOnWriteArrayList<FactoryElement> fel) {
